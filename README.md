# 🔧 SP Item Editor - The Cycle: Reborn Save Editor

<div align="center">

![Version](https://img.shields.io/badge/version-2.0.0-brightgreen)
![Platform](https://img.shields.io/badge/platform-Windows-blue)
![License](https://img.shields.io/badge/license-MIT-orange)

**A powerful desktop application for editing The Cycle: Reborn save files through MongoDB**

[📥 **Download Latest**](#-download) | [🐛 Report Bug](https://github.com/ConstanceV1/SingePlayer-Item-editor/issues) | [💡 Request Feature](https://github.com/ConstanceV1/SingePlayer-Item-editor/issues)

</div>

---

## 📥 Download

### Latest Release: v2.0.0

| Version | Download |
|---------|----------|
| **Installer** | [Download Setup EXE](https://github.com/ConstanceV1/SingePlayer-Item-editor/releases/latest/download/TheCycleReborn-SaveEditor-Setup-v2.0.0.exe) |
| **Portable** | [Download Portable EXE](https://github.com/ConstanceV1/SingePlayer-Item-editor/releases/latest/download/TheCycleReborn-SaveEditor-v2.0.0.exe) |

---

## ✨ Features

### 🎒 Inventory Management
- **Edit Stash Items** – Add, remove, or modify any item in your stash
- **Smart Stacking** – Automatically handles item stack limits and splitting
- **Item Search** – Quickly find items by name or ID
- **Category Filtering** – Filter by weapons, ammo, armor, consumables, materials, tools, and attachments
- **Visual Interface** – Beautiful item icons with rarity-based color coding

### 💰 Currency Editor
- **Edit Aurum** (Premium currency)
- **Edit K-Marks** (Standard currency)
- **Edit Insurance Credits**
- **Real-time Updates** – Changes save automatically

### 🔧 Advanced Features
- **Item Durability** – Adjust weapon and armor durability
- **Stack Management** – Handles proper item stacking automatically
- **Backup & Restore** – Export and import your save data
- **Auto-Save** – Changes are automatically saved to your database
- **Game Launcher** – Launch The Cycle: Reborn directly from the editor

### 🎨 User Experience
- **Modern Glass UI** – Sleek, cyberpunk-inspired interface
- **Responsive Design** – Clean layout that scales to your screen
- **Toast Notifications** – Instant feedback for all actions
- **Item Details** – Click items to view and edit detailed information

---

## 🚀 Quick Start

### Step 1: Download
Download the latest EXE from the [Releases](https://github.com/ConstanceV1/SingePlayer-Item-editor/releases) page.

### Step 2: Install MongoDB
Download and install MongoDB Community Edition from [mongodb.com](https://www.mongodb.com/try/download/community)

### Step 3: Run the Editor
1. **Run** `The Cycle Reborn Save Editor.exe`
2. **Auto-Connect** – The application will automatically connect to your MongoDB
3. **Start Editing** – Your inventory will load automatically!

---

## 📖 How to Use

### Adding Items
1. Click **"Browse Items"** in the sidebar
2. Use **category filters** or **search** to find items
3. Set the **quantity** you want
4. Click **"Add"** to add items to your stash

### Editing Items
1. **Click on any item** in your inventory
2. **Modify quantity** or **durability**
3. Click **"Update"** to save changes
4. Or click **"Remove"** to delete the item

### Currency Editing
1. **Click on any currency value** in the top-right
2. **Type the new amount** you want
3. **Press Enter** – changes save automatically!

---

## ⚠️ Important Notes

### Safety First
- 🔒 **Always backup your save** before making major changes
- 🎮 **Close the game** before using the editor
- 💾 **Auto-save is enabled** – changes are saved automatically

### Requirements
- ✅ **Windows 10/11**
- ✅ **MongoDB 4.0+** running locally
- ✅ **The Cycle: Reborn** installed

---

## 🛠️ Troubleshooting

<details>
<summary><strong>❌ "Connection Failed" Error</strong></summary>

1. **Install MongoDB** – Download from [mongodb.com](https://www.mongodb.com/try/download/community)
2. **Start MongoDB Service** – Check Windows Services for "MongoDB Server"
3. **Check Port 27017** – Ensure MongoDB is running on default port
</details>

<details>
<summary><strong>📦 "No Inventory Found" Error</strong></summary>

1. **Play the game first** – Create a character and enter the game once
2. **Check database** – Ensure ProspectDb exists in MongoDB
3. **Try reloading** – Click the reload button in the header
</details>

---

## 👏 Credits

| Role | Name |
|------|------|
| **Original Creator** | [Lxjo](https://github.com/lxjo101) – Built the foundation and core functionality |
| **Maintainer & Developer** | [Constance](https://github.com/ConstanceV1) – UI overhaul, bug fixes, feature improvements, and ongoing development |

**Special Thanks**
- The Cycle: Reborn community for feedback and support
- Contributors who report bugs and suggest features

---

## 🤝 Contributing

Found a bug or want to suggest a feature?

- 🐛 **Report bugs** in the [Issues section](https://github.com/ConstanceV1/SingePlayer-Item-editor/issues)
- 💡 **Suggest features** or improvements
- 🔧 **Submit pull requests** with fixes or enhancements

---

## 📄 License

This project is licensed under the **MIT License** – see the [LICENSE](LICENSE) file for details.

---

## ⚡ Disclaimer

**This is an unofficial tool** created by the community for The Cycle: Reborn players.

- ✅ **Use at your own risk** – Always backup your saves
- ✅ **Single-player focused** – Designed for offline/local play
- ✅ **No game modification** – Only edits save data through standard database access

---

<div align="center">

**Made with ❤️ by the community**

⭐ **Star this repo** if you find it useful! ⭐

[Discord](https://discord.gg/gvgbca8yxz) · [GitHub](https://github.com/ConstanceV1)

</div>
